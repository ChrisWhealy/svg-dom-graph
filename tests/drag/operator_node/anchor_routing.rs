//! The same-side anti-crossing split between a two-input operator's own operands, `EdgeAnchors` honoured on its
//! auto-wired operand edges, and dragging either an operand or the operator itself.
...
use svg_dom_graph::scene::{
    BinaryOperator, DataFormat, DataNodeContent, EdgeAnchors, NodeOptions, NodeValues, Scene, UnaryOperator,
};
