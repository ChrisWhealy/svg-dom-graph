...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
/// Where the demo's staged `index.html`, `style.css`, and wasm package live (see `main`'s own doc comment for the full
/// directory layout). Bundled into one struct since every phase below needs some subset of these same paths, derived
...
/// variant it gets via `Display` and exits — see this module's own doc comment for why staging itself never does that.
#[derive(Debug)]
pub enum BuildError {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
/// Runs [`prepare_stage`] and then rebuilds the wasm package, leaving only starting the Actix server itself to `main`.
/// This is the full pipeline `cargo demo` needs; `main --prepare-only` runs [`prepare_stage`] alone instead (see this
/// module's own doc comment for why that split exists).
