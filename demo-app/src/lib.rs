...
//! - `panel-operators` / `#operators-diagram` — [`build_operator_demo`]: one operand node (or two, for a binary
//!   operator) feeding an operator node, for every unary and binary operator this crate names. Every result shown
//!   is computed here with plain Rust integer ops, never by the library itself — see that function's own doc
//!   comment.
//! - `panel-selection` / `#selection-1d-diagram` and `#selection-2d-diagram` — [`build_selection_demo`]: a
//!   one-dimensional and a two-dimensional array, each with its own "Previous"/"Next" buttons stepping
//!   [`Scene::set_selection`] through its values. See that function's own doc comment for what each demonstrates.
//!
...
use svg_dom_graph::{
    EdgeId, NodeId,
    scene::{
        BinaryOperator, ConnectorOptions, ConnectorType, DataFormat, DataNodeContent, DragOptions, EdgeAnchors,
        GridLayout, NodeOptions, NodeValues, Scene, Selection, UnaryOperator,
    },
};
...
thread_local! {
    ...
    // Same reasoning, for `build_operator_demo`'s own, separate `Scene`.
    static OPERATOR_SCENE: RefCell<Option<Scene>> = const { RefCell::new(None) };
    // Same reasoning, for `build_selection_demo`'s own two, separate `Scene`s — one per array dimension.
    static SELECTION_SCENE: RefCell<Option<(Scene, Scene)>> = const { RefCell::new(None) };
}
...
demo_gallery! {
    ...
    "panel-operators" => build_operator_demo,
    "panel-selection" => build_selection_demo,
}
...
fn build_operator_demo() -> Result<(), String> {
    ...
    // Keeps this Scene's only strong handle alive for the page's lifetime — see OPERATOR_SCENE's own doc comment.
    OPERATOR_SCENE.with_borrow_mut(|slot| *slot = Some(scene));
...
/// `# Errors` section).
fn build_selection_demo() -> Result<(), String> {
    let document = document()?;

    let one_d_values: Vec<u8> = vec![10, 20, 30, 40, 50, 60];
    let one_d_len = one_d_values.len();
    let one_d_svg = SvgRoot::attach("selection-1d-diagram").map_err(stringify)?;
    ...
    let two_d_svg = SvgRoot::attach("selection-2d-diagram").map_err(stringify)?;
    ...
    // Keeps both Scenes' only strong handle alive for the page's lifetime — see SELECTION_SCENE's own doc comment.
    SELECTION_SCENE.with_borrow_mut(|slot| *slot = Some((one_d_scene.clone(), two_d_scene.clone())));
...
/// `0..demo.one_d_len` by `step_one_d` below, so this call cannot fail in practice. A live button handler should
/// never panic or stop responding over a stray, already-impossible error.
...
/// Applies `demo`'s own current `two_d_index` as a [`Selection::Row`] — the row it falls in, banded, and its own
/// column within that row focused — and shows both in `output`. Same "cannot fail in practice" reasoning as
...
/// `Closure::forget` leaks them deliberately, for the page's whole lifetime — the same span `SELECTION_SCENE`
/// itself covers.
///
/// # Errors
///
