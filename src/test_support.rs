...
//! (non-`--test`) build in the first place. `tests/drag/common.rs` keeps its own copy of [`check`] for that reason, not
//! because the duplication was overlooked.

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
/// Returns `Err(msg)` when `condition` is `false` — every test in this crate, internal or external, follows the same
/// `Result<(), String>` convention: a failure prints its message directly, with no panic and no stack trace.
