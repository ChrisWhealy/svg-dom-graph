...
//!
//! Not reachable from the external `tests/drag.rs` integration test binary: that binary depends on this crate as
//! any external consumer would, so it only sees the public API, and `#[cfg(test)]` code is not part of a normal
//! (non-`--test`) build in the first place. `tests/common.rs` keeps its own copy of [`check`] for that reason,
//! not because the duplication was overlooked.
